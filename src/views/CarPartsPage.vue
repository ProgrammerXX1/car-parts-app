            ...
            <template v-for="part in flattenParts(bodyParts)" :key="part.id">
              <tr>
                ...
                <td class="text-center">
                  ...
                  <button class="btn btn-secondary btn-sm" @click="incrementValue(part.id, 1)">Прибавить</button>
                </td>
              </tr>
            </template>
          </tbody>
...
export default defineComponent({
  ...
  computed: {
    ...
    bodyParts(): Part[] {
      return this.parts.filter((part) => part.name === 'Кузов');
    },
    // Фильтруем детали двигателя
    engineParts(): Part[] {
      return this.parts.filter((part) => part.name === 'Двигатель');
    },
  },
  methods: {
    ...
    addNewPart(type: string) {
      ...
      if (type === 'body') {
        ...
        const bodyPart = this.parts.find((part) => part.name === 'Кузов');
        ...
        const enginePart = this.parts.find((part) => part.name === 'Двигатель');
    ...
    // Увеличиваем количество детали на указанное значение (например, +1)
    ...
    exportToPDF() {
      ...
      let yPosition = 20;
      this.flattenParts(this.parts).forEach((part) => {
        doc.text(`Деталь: ${part.name}, Цена: ${part.price}, Количество: ${part.quantity}`, 10, yPosition);
        yPosition += 10;
      });
